...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward_encoder(self, x, mask):
        ...
        x, ids_restore = self.adaptive_masking(x, mask)
        # append cls token
    ...
    def forward_loss(self, imgs, pred, mask):
        ...
        adaptive_mask = torch.ones([N, L], device=mask.device)   # ascend: small is keep, large is remove
        adaptive_mask[:, :len_keep] = 0
        # unshuffle to get the binary mask
        adaptive_mask = torch.gather(adaptive_mask, dim=1, index=ids_restore)

        return loss, adaptive_mask

    def forward(self, imgs, mask):
        latent, ids_restore = self.forward_encoder(imgs, mask)
        pred = self.forward_decoder(latent, ids_restore)  # [N, L, p*p*3]
        loss, adaptive_mask = self.forward_loss(imgs, pred, mask)
        return loss, pred, mask, adaptive_mask
